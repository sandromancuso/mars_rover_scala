...
class MarsRoverShould extends UnitSpec {
	...
	"wrap around through the north side of the grid" in new context {
		override val marsRover = new MarsRover(GRID_10_x_10, Position(9, 9, North))

		marsRover execute("M")

		marsRover.position should be(Position(9, 0, North))
	}

	"wrap around through the west side of the grid" in new context {
		override val marsRover = new MarsRover(GRID_10_x_10, Position(0, 0, West))

		marsRover execute("M")

		marsRover.position should be(Position(9, 0, West))
	}

	"wrap around through the east side of the grid" in new context {
		override val marsRover = new MarsRover(GRID_10_x_10, Position(9, 9, East))

		marsRover execute("M")

		marsRover.position should be(Position(0, 9, East))
	}
}
